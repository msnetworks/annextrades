                ...
                if ($data->status == 1) {
                    ...
                    return'<div class="action-list">
                            <select class="process select droplinks '.$class.'">
                                <option data-val="2" value="'. route('vendor-prod-status',['id1' => $data->id, 'id2' => 2]).'" '.$s.'>'.$this->vendor_language->lang713.'</option>
                                <option data-val="0" value="'. route('vendor-prod-status',['id1' => $data->id, 'id2' => 0]).'" '.$ns.'>'.$this->vendor_language->lang714.'</option>';
                            '</select></div>';
                }
                else {
                    $class = $data->status == 2 ?  'drop-warning' : 'drop-danger' ;
                    $s = $data->status == 2 ? 'selected' : '';
                    $ns = $data->status == 0 ? 'selected' : '';
                    return '<div class="action-list">
                        
                        <select class="process select droplinks '.$class.'">
                            <option data-val="2" value="'. route('vendor-prod-status',['id1' => $data->id, 'id2' => 2]).'" '.$s.'>'.$this->vendor_language->lang1004.'</option>
                            <option data-val="0" value="'. route('vendor-prod-status',['id1' => $data->id, 'id2' => 0]).'" '.$ns.'>'.$this->vendor_language->lang714.'</option>';
                        '</select></div>';
                } $class = $data->status == 1 ? 'drop-success' : 'drop-danger';
                $s = $data->status == 2 ? 'selected' : '';
...
/*         $rules =
        ...
        if ($validator->fails()) {
            return back()->with(array('errors' => $validator->getMessageBag()->toArray()));
        } */
        $insert = DB::table('quotes_submit')->insert(['user_id' => Auth::user()->id, 'buyer_id' => $request->buyer_id, 'quote_id' => $request->quote_id, 'item_name' => $request->item_name, 
        ...
        'shipping' => $request->shipping, 'taxes' => $request->taxes, 'total' => $request->total, 'quote_terms' => $request->quote_terms, 'signature' => $request->newsignature, 'status' => 1]);
        
         
        if (!$insert) {
            return back()->with(array('errors' => [ 0 => 'insert error' ]));
        }
        else {
            
            return redirect()->route('vendor-requests')->with('success', 'You quote was successfully submitted. Responses from Buyer will be directed to your Quote Request');
        }
