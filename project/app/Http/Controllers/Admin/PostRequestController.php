...
{   
    ...
    {
        return view('admin.PostRquirement.index');
    }
        ...
        //--- Integrating This Collection Into Datatables
         return Datatables::of($data)
                            ->editColumn('name', function($data) {
                                if($data->deadline >= date("Y-m-d H:i:s")){
                                    $date = '<div class="text-success">Deadline : '.$data->deadline.'</div>';
                                }else{
                                    $date = '<div class="text-danger">Deadline : '.$data->deadline.'</div>';
                                }
                                $name = mb_strlen(strip_tags($data->name),'utf-8') > 50 ? mb_substr(strip_tags($data->name),0,50,'utf-8').'...' : strip_tags($data->name);
                                $id = '<small>ID: <a href="user/'.$data->user_id.'/show" target="_blank">'.sprintf("%'.08d",$data->user_id).'</a><br> '.$date.'</small>';
                                $name = $name."<br>".$id;
                            ...
                            ->editColumn('product_name', function($data) {
                                ...
                                $pro = $product_name."<br>".$id;
                                return  mb_convert_encoding($pro, 'UTF-8', 'UTF-8');
                            })
                            ->editColumn('short_des', function($data) {
                                ...
                                return  mb_convert_encoding($short_des, 'UTF-8', 'UTF-8');
                            })
                            ->editColumn('product_des', function($data) {
                                ...
                                return  mb_convert_encoding($product_des, 'UTF-8', 'UTF-8');
                            })
                            ...
                            ->editColumn('price_range', function($data) {
                                ...
                                return  mb_convert_encoding($price, 'UTF-8', 'UTF-8');
                            })
                            ->editColumn('quote_nos', function($data) {
                                ...
                                return  mb_convert_encoding($count, 'UTF-8', 'UTF-8');
                            })
                            ->editColumn('mail', function($data) {
                                $mail = '<i class="fa fa-envelope"></i>'; 
                                return  mb_convert_encoding($mail, 'UTF-8', 'UTF-8');
                            })
                            ...
                            ->addColumn('delete', function($data) {
                                ...
                                return mb_convert_encoding($button, 'UTF-8', 'UTF-8');
                            })
                            ->rawColumns(['name', 'company_name','product_name','short_des','product_des','email','phone','price_range', 'icon', 'notify','view','mail', 'quote_nos', 'status', 'highlight', 'delete'])
    ...
    public function add_new()
    {
        ...
        return view('admin.PostRquirement.add_newquote', $data);
    }
    ...
    {   
        ...
        $insert = DB::table('PostRequest')->insert(['user_id' => $request->user_id, 'request_id' => $request_id, 'product_name' => $request->product_name, 
        'company_name' => $request->company_name, 'cover_image_category' => $request->cover_image_category, 'product_des' => $request->product_des, 'short_des' => $request->short_des, 'pri_gov' => $request->pri_gov, 'price_from' => $request->price_from, 
        'photo' => $image, 'photo1' => $image1, 'photo2' => $image2, 'photo3' => $image3, 'photo4' => $image4, 'deadline' => $request->deadline, 'name' => $request->name,
    ...
    public function edit($id)
    {
        ...
        return view('admin.PostRquirement.edit')->with('data', $data)->with('all_category', $all_category);
    }
    public function update(Request $request)
    {
        ...
        $update = DB::table('PostRequest')->where('id', $request->id)->update(['company_name' => $request->company_name, 'cover_image_category' => $request->cover_image_category, 'type' => $request->type, 
