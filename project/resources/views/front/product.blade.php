...
<section class="product-details-page lr-30">
  <div class="container-fluid">
    <div class="row">
    <div class="col-lg-9">
        <div class="row">
          ...
          <div class="xzoom-container">
            ...
            <div class="product-price">
              <p class="title">{{ $langg->lang87 }} :</p>
                    <p class="price"><span id="sizeprice">{{ $productt->showPrice() }} 
                      <small><del>{{ $productt->showPreviousPrice() }}</del></small></span> &nbsp;&nbsp;<span style="color: #143250; font-size: 16px; font-weight: 600;">{{ $productt->measure }}</span></p>
                  ...
                  <div class="info-meta-3">
                    <ul class="meta-list">
                      ...
                      <li class="addtocart">
                        ...
                        @if (Auth::guard('web')->user()->id == $productt->user_id)
                          <a href="javascript:;" onclick="alert('You cannot Submit Quote to your own products.')"><i class="icofont-cart"></i>{{ __('Get Quote') }}</a>
                        @else
                          <a href="javascript:;" data-toggle="modal" data-target="#buyBulk"><i class="icofont-cart"></i>{{ __('Get Quote') }}</a>
                        @endif
                        @else
                          <a href="javascript:;" onclick="alert('Login for request.')" data-toggle="modal" data-target="#comment-log-reg"><i class="icofont-cart"></i>{{ __('Get Quote') }}</a>
                        @endif
                            <!-- Modal -->
                          <div class="modal fade" id="buyBulk" tabindex="-1" role="dialog" aria-labelledby="buyBulkTitle" aria-hidden="true">
                                  ...
                                  <div class="gocover" style="background: url({{ asset('assets/images/'.$gs->loader) }}) no-repeat scroll center center rgba(45, 45, 45, 0.5);"></div>
                                    <div class="login-area">
                                        <div class="header-area forgot-passwor-area">
                                            <h4 class="title">{{ __('Request For Buy Bulk') }}</h4>
                                            <p class="text">{{ $langg->lang778 }}</p>
                                        </div>
                                        <div class="login-form"> 
                          
                                            <form id="buybulk" class="mregisterform" action="{{ route('buy-bulk') }}" method="POST">
                                              ...
                                              @include('includes.admin.form-login')
                                                ...
                                                <div class="form-input">
                                                    <input type="text" name="shop_name" class="User Name" value="{{ $productt->user->shop_name }}" required="">
                                                    <i class="icofont-user"></i>
                                                </div>
                                                <div class="form-input">
                                                    <input type="text" name="product_name" class="User Name" value="{{ $productt->name }}" required="">
                                                    <i class="icofont-notepad"></i>
                                                </div>
                                                <div class="form-input">
                                                    <input type="number" name="quantity" class="User Name" placeholder="{{ __('Required Quantity') }}" required="">
                                                    <i class="icofont-tag"></i>
                                                </div>
                          
                                                <div class="form-input">
                                                  <textarea name="note" class="User Name" placeholder="{{ __('Message') }}" required=""></textarea>
                                                </div>
                  ...
                  <div id="product-details-tab">
                    ...
                    <div class="tab-content-wrapper">
                      ...
                      <div id="tabs-2" class="tab-content-area">
                        <p>{!! html_entity_decode($productt->policy) !!}</p>
                      </div>
           ...
           @if( $productt->user_id  != 0)
              @if(isset($productt->user))
                ...
                <a class="{{-- verify-link --}}" href="javascript:;"  data-toggle="tooltip" data-placement="top" title="{{ $langg->lang783 }}">
                  ...
                  <img class="img-flag2" width="80px" src="{{ asset('assets/images/flag/verifiedsheld.png') }}" alt="Verified Seller">
                </a>
                @endif
              <br><a href="#" data-toggle="modal" data-target="#bd-example-modal-lg">{{ $productt->user->shop_name }}</a>
              {{-- SHOP INFO --}}
                <!-- Modal -->
                <div class="modal fade bd-example-modal-lg" id="bd-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
                  <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                      <div class="modal-body">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                          @if ($productt->user->shop_details)
                            <h2 class="title" style="font-size: 22px"><b>About Company</b></h2>
                            <p>{!! html_entity_decode($productt->user->shop_details) !!}</p>
                          @else
                            <h2 class="text-center">No Detail Found</h2>
                          @endif
                      </div>
                    </div>
                  </div>
            ...
            <span>{{ $langg->lang248 }}</span>
                  ...
                  <div class="contact-seller">
                    ...
                    <ul class="list">
                      ...
                      @if( $productt->user_id  != 0)
                        <li><a href="{{ route('front.vendor',str_replace(' ', '-', $productt->user->shop_name)) }}" class="view-stor">{{ $langg->lang249 }}</a></li>
                      @endif
                      @if(Auth::guard('web')->check())
                    ...
                    <ul>
                      ...
                      <li>
                        <input type="text" class="input-field" id="subj" name="subject"
                          placeholder="{{ $langg->lang119}}" value="{{ $productt->name }}" required="">
                      </li>
                      ...
                      <li>
                        {{-- <small>Send to all seller with related category.</small><br> --}}
                        <input class="input-field" type="checkbox" value="1" name="allseller" id="allseller"><label for="allseller">&nbsp; Send to all seller with related category.</label> &nbsp;&nbsp;
                        {{-- <input type="radio" value="0" name="allseller" id="allsellerno" checked ><label for="allsellerno">&nbsp; No</label> --}}
                      </li>

                      <input type="hidden" name="email" value="{{ Auth::guard('web')->user()->email }}">
                      <input type="hidden" name="subcategory_id" value="{{ $productt->subcategory_id }}">
...
<script type="text/javascript">
  ...
  $(document).on("submit", "#emailreply", function () {
    ...
    var allseller = $(this).find('input[name=allseller]');
    if (allseller.is(':checked') === false) {
      var allseller = 0;
    } else {
      var allseller = 1;
    }
    var subcategory_id = $(this).find('input[name=subcategory_id]').val();
