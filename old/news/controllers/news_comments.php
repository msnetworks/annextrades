        ...
        if (strlen($comment) < 2) {
            ...
            if (strlen($author) < 2) {
                ...
                if ($spam) { // Honeypot - Stop spam bots
                    ...
                    $runq = mysqli_query($connect, "INSERT INTO `comments` (post_id, `comment`,author,email, date, time) VALUES ('$id', '$comment', '$author','$email', '$date', '$time')");
                    //  var_dump($connect, $runq);
                    if ($runq) {
                        echo "<script>location.href='../news_details.php?id=$id&action=Success';</script>";
                    }
                    else {
                        echo "<script>location.href='../news_details.php?id=$id&action=Success';</script>";
                    }
