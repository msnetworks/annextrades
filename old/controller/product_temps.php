...
for($i = 0 ; $i < $size ; $i++){
    ...
    if (!empty($items)) {
        ...
        foreach($items as $item){
            $values[] = "('$vendor_id','$usr_id','{$item['p_name']}', '{$item['quantity']}', '{$item['p_price']}', '{$item['product_des']}', '{$item['photo']}', '{$item['photo1']}', '{$item['photo2']}', '{$item['photo3']}', '{$item['photo4']}')";
            }
        }

        $values = implode(", ", $values);

        $sql = "INSERT INTO product_temp (userid, p_name, p_min_quantity, p_price, p_ddes, p_photo, photo1, photo2, photo3, photo4) VALUES  {$values} " ;
