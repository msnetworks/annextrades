...
//session_start();
	...
	$sqlget=mysqli_query($con,"SELECT * FROM faq_question");
	/*$sqlget1=mysqli_query($con,"SELECT * FROM faq_question_french");
	$sqlget2=mysqli_query($con,"SELECT * FROM faq_question_chinese");*/
	
	if(isset($_REQUEST['delid']))
	{
	$delid=$_REQUEST['delid'];
	$sql=mysqli_query($con,"DELETE FROM faq_question WHERE id='$delid'");
	
	$sql=mysqli_query($con,"DELETE FROM faq_question_french WHERE id='$delid'");
	
	//$sql=mysqli_query($con,"DELETE FROM faq_question_chinese WHERE id='$delid'");
	
	$sql=mysqli_query($con,"DELETE FROM faq_question_spanish WHERE id='$delid'");
	
	header("Location:faq.php?&del=suc");
	}
	if(isset($_REQUEST['submit']))
	{
	$buy_over=$_POST['buy_over'];
	...
	$sql=mysqli_query($con,"INSERT INTO general(`buy_over`)value('$buy_over')");
	}
	header("Location:buy_over.php");
	}
?>
		...
		<div class="tab_container">
			...
			<table width="98%" align="left" cellspacing="0" >
				...
				<tr><td valign="top">
					...
					<table width="615" height="70" align="center" style="border:1px solid #E6E6E6;">
						<tr bgcolor="#E6E6E6">
						  <td colspan="2" height="20"><div align="center"><strong>Faq Subject </strong></div></td>
					  </tr>
						<tr>
						  <td width="44" class="gboldli">&nbsp;</td>
						  <td width="559" align="right"><a href="addnewfaq.php" class="gboldli" style="color:#000099;"><b>Add new</b></a></td>
					  </tr>
					 ...
					 <!-- <?php //while($row=mysqli_fetch_assoc($sqlget)) { ?>
						<tr>
						  <td class="gboldli">&nbsp;</td>
						  <td><a href="related_question.php?id=<?php //echo $row['id']; ?>"><?php //echo $row['question']; ?></a></td>
					  </tr>
					  <?php //} ?>-->
						<tr><td class="gboldli">&nbsp;</td>
						  <td><table width="559" height="12">
						    <tr>
						      <td width="461"><strong>Subject</strong></td>
						      <td width="27"><strong>Edit</strong></td>
						      <td width="55"><strong>Delete</strong></td>
					        </tr>
							<?php while($row=mysqli_fetch_assoc($sqlget)) { ?>
						    <tr>
						      <td><a href="related_question.php?id=<?php echo $row['id']; ?>" class="news" style="color:#000099;"><b><?php echo $row['question']; ?></b></a></td>
						      <td><a href="addnewfaq.php?editid=<?php echo $row['id']; ?>"><img src="images/images (1).jpg" style="width:17px; height:17px;"/></a></td>
					          <td><a href="faq.php?delid=<?php echo $row['id']; ?>" onclick="return confirm('Are you sure do you want to delete this record?');"><img src="../images1/delete.jpg" alt="delete" title="delete" border="0" /></a></td>
							</tr>
							 <?php } ?>
							</table></td>
						</tr>
