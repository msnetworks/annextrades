<?php 
	...
	{
		$selected_friends = $_POST['checklist']; 
		foreach($selected_friends as $sel)
		{     
			$sel;  
			mysqli_query($con,"UPDATE `messages` SET `fromstatus`='1' WHERE `id`=$sel");
		}   
...
<form action="" method="post"  name="sent" id="sent">
<input type="hidden" name="opendiv" id="sentopendiv" value="">
...
<div>
		<?php 
			...
			$select="select * from `messages` where  from_mail='$mail' and fromstatus='0' and user_id='$session_user' order by `id` desc";
			...
			$i=0;
		?>
	<table border="0" width="95%" align="center" cellpadding="0" cellspacing="0" style="margin-top:8px;">
	...
	<?php if($count_msg>0) 
			{ 
			?>
            <tr bgcolor="#97DDFF" height="25">
		<th width="23%"  style="border-top-left-radius:10px;"><a href="#" class="topics" onclick="javascript:SetAllCheckBoxes('sent', 'checklist[]', true)"><?php echo $select_all; ?></a>&nbsp;/&nbsp;<a href="#" class="topics" onclick="javascript:SetAllCheckBoxes('sent', 'checklist[]', false)"><?php echo $clear_all; ?></a><?php } ?></th>
		<!-- <th width="29%"><?php echo $sender; ?></th> -->
		<th width="25%"><?php echo $subject; ?></th>
		<th width="23%"  style="border-top-right-radius:10px;"><?php echo $date; ?></th>
        </tr>
		<?php 
			if($count_msg>0)
			{
		...
		?>
		<tr height="25"  >
			<td align="center" style=" border-left:solid 1px #97DDFF; "><input type="checkbox" name="checklist[]" value="<?php echo $array_msg['id']; ?>" id="checklist[<?php echo $i; ?>]" style="line-height:25px;" /></td>
			<!-- <td align="center" style="line-height:30px;"><?php echo $array_msg['to_mail']; ?></td> -->
			<td align="center" style="line-height:30px;">
				<a href="sent_items_view.php?view=<?php echo $array_msg['id']; ?>">
					<div><b><?php echo $array_msg['subject']; ?></b></div>
					<div class="email__desc"><?php echo $array_msg['message']; ?></div>
				</a>
			</td>
			<td align="center" style="line-height:30px;border-right:solid 1px #97DDFF;"><?php echo $array_msg['date']; ?></td>
		</tr>
		...
		<tr>
			<td>
				<td style="color:#C55000;" align="center"><b><?php echo $no_record; ?></b></td>
			</td>
		</tr>
		<?php } ?>
		<tr>
			<td align="center">
			<?php if($count_msg>0) { $i=0; ?>
			<input type="submit" name="sent" value="<?php echo $delete; ?>" class="search_bg" onclick="return compare1();" style="margin-top:20px;"/>
			<?php } ?>
			</td>
		</tr>
		<tr>
		<table ><tr align="right"> 
   			<td  colspan="12" align="right"><div style="text-align:right"><?php echo  $pagingLink;?></div></td> 
    	</tr></table>
		<tr>
	</table>
