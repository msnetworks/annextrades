            ...
            <div class="tabs-cont" style="margin-top:0">
                <div class="left">
                    <div class="p-2" style="padding-top:0">
                        <div class="products-cate-heading"><h5><?php echo $privacy; ?></h5>
                        </div>
                        <table style="line-height: 1.5">
                            <form id="form1" name="form1" method="post" action="">
                                <?php 
                                    ...
                                    {
                                    $select=mysqli_query($con,"select privacy_policy from cms");
                                    }
                                    else if($_SESSION['language']=='french')
                                    {
                                    $select=mysqli_query($con,"select privacy_policy from cms_french");
                                    }
                                    else if($_SESSION['language']=='chinese')
                                    {
                                    $select=mysqli_query($con,"select privacy_policy from cms_chinese");
                                    }
                                    else
                                    {
                                    $select=mysqli_query($con,"select privacy_policy from cms_spanish");
                                    }	
                                        $policy=mysqli_fetch_array($select);
                                ?>
                                <tr>
                                    <td style="text-align:justify;padding-left:10px;" valign="top">
                                        <?php echo $policy['privacy_policy']; ?></td>
                                </tr>
                            </form>
                        </table>
                    </div>
