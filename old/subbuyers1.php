...
<div class="products-cate-heading"> <?php echo $sub_cat; ?><span style="float:right;"><a href="addbuying_leads.php" style="font-weight:bold;"><?php echo $add_buy; ?></a></span> </div>
<?php
if($_SESSION['language']=='english')
{
$query ="select * from category where parent_id='$_REQUEST[cat_id]' order by category";
}
else if($_SESSION['language']=='french')
{
$query ="select * from category_french where parent_id='$_REQUEST[cat_id]' order by category";
}
else if($_SESSION['language']=='chinese')
{
$query ="select * from category_chinese where parent_id='$_REQUEST[cat_id]' order by category";
}
else
{
$query ="select * from category_spanish where parent_id='$_REQUEST[cat_id]' order by category";
}
...
$pagingLink = getPagingLink($query, $rowsPerPage, "cat_id=$_REQUEST[cat_id]");
...
$querycount=mysqli_query($con,"select * from buyingleads where category='$sub1' and status='2' ");
...
<div class="procate-heading"><a href="selling_buy_leads1.php?subcat_id=<?php echo $fetch['c_id']; ?>" class="tradenews" style="padding-left:10px;"><?php echo $fetch['category'];  ?></a></div>


<div class="procate-cont-img"><a href="selling_buy_leads1.php?subcat_id=<?php echo $fetch['c_id']; ?>" class="tradenews" ><?php if(($fetch['cat_image'] == "") || (!file_exists("admin/category_images/".
$fetch['cat_image'] )) ) { ?>
					<img src="images/img_noimg.jpg" width="74" height="74" />
					<?php } else { ?>
					<img src="admin/category_images/<?php echo $fetch['cat_image']; ?>" width="74" height="74"/>
					<?php } ?></a></div>
<div class="procatelist">
<?php
if($co>0)
{}  else {?> <?php //echo $nocat; ?> <?php } ?>
...
<div class="line"></div>
<div style="clear:both"><?PHP echo $pagingLink;
     echo "<br>";?>
					  </div>
