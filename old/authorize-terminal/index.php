...
//important file. Don't forget to edit it!
...
if ( ! empty( $_POST["process"] ) && $_POST["process"] == "yes" ) {
    $qr = @mysqli_query($con, "INSERT INTO ccdetails SET vendor_id='".$_POST['vndor']."', amount='$amount', service='$service' ");
	$captcha = false;
	if ( $enableReCaptcha ) {
		$ch = curl_init();
...
<!-- <h1>Authorize.net Payment Terminal</h1> -->
                    ...
                    <div align="center" class="wrapper">
                        <?php include "includes/javascript.validation.php"; ?>
                            <div class="form_container">
                                ...
                                class="pppt_form">
                                <input type="text" name="vendor_id" value="<?php echo $vendor_id; ?>" hidden/>
                                <input type="text" name="add_amt" value="<?php echo $p; ?>" hidden/>
                                <div id="accordion" >
                                    <?php if ( $show_form ) { ?>
                                        <!-- PAYMENT BLOCK -->
                                        <h2 class="current">Payment Information</h2>
                                        <div class="pane" style="display:block" id="detdiv">
                                            <?php if ( $show_services || $payment_mode == "RECUR" ) {
                                                echo "<label>Service: </label><select name='service' id='service' class='long-field' onchange='checkFieldBack(this)'>";
                                                switch ( $payment_mode ) {  
                                                    ...
                                                    case"RECUR":
                                                        ...
                                                        foreach ( $recur_services as $k => $v ) {
                                                            echo "<option id='p' value='" . $k . "' " . ( $service == $k ? "selected" : "" ) . ">" . $v[0] . " ($" . number_format( $v[1], 2 ) . ")" . "</option>";
                        ...
                        <div class="form_container">
                            ...
                            <div class="col-md-12 p-0" style="background: #ff790042;">
                                <label style="padding-left: 15px; font-size: 18px; display: inline;">
                                    <input type="int" name="vndor" value="<?php echo $vendor_id;?>" hidden/>
                                    <input type="radio" name="add_amt" id="add_amt" <?php if ($rate != "316.91"){ echo "checked"; };?> onclick="document.getElementById('frm').submit();"  value="375.91" required> Yes, add an U.S. Address = $59/mo
                                </label>
                            </div>  
                            <div class="col-ms-12">
                            ...
                            <div class="col-md-12 p-0" style="">
                                <label style="padding-left: 15px; display: inline;">
                                    <input type="radio" name="add_amt" <?php if ($rate != "375.91"){ echo "checked"; };?> value="316.91" onclick="document.getElementById('frm').submit();" id="add_amt1" required> No, I decline protection and accept responsibility for damage.
                                </label>
                            </div>
                            <div class="col-md-12" style="padding-top: 15px;">
                                <div>
                                    <h4 class="pull-left text-left"><b>Total Amount <!-- after Trail -->:</b></h4>
                                    <h4 class="h4ull-right text-right">$<input type="text"  name="rate" <?php if ($rate != "375.91"){ echo "value='316.91'"; }else{echo "value='375.91'";};?>   id="newrate" style="border: 0; text-align: right; outline: none; width: 70px;" readonly></h4>
                                </div>
                            </div>
        ...
        <script type="text/javascript">
            ...
            $("#add_amt").change(function() {
                var add_amt = document.forms['ff1']['rate'].value;
                
                var data = $(this).serialize();
                console.log(add_amt);
                    if (add_amt == '316.91') {
                        document.forms['ff1']['rate'].value = '375.91';
                        $('#rs').html('375.91');
                        $('#prc').html('375.91');
                        $("#bkrs").load(" #bkrs > * ");
                        $("#detdiv").load(" #detdiv > *");
            ...
            $("#add_amt1").change(function() {
            var add_amt = document.forms['ff1']['rate'].value;
            var data = $(this).serialize();
                console.log(add_amt);
                    if (add_amt == '375.91') {
                        document.forms['ff1']['rate'].value = '316.91';
                        $('#rs').html('316.91');
                        $("#bkrs").load(" #bkrs > * ");
                        $("#detdiv").reload(" #detdiv > *");
                    }
                });
