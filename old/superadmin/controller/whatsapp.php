...
include('../../controller/config.php');
    ...
    $lot = $_POST['lot'];
 $query = $conn->query("SELECT * FROM whatsapp WHERE lot = '$lot' ORDER BY id DESC LIMIT 30000");
 ...
 WHILE($row=mysqli_fetch_array($query)){
    ...
    $conn->query("INSERT INTO whatsapp_report (`phone`, `status`, `message_id`, lot) VALUE ('$number', '$status', '$msg', '$lot')");
}
echo "<script>window.location.href='../whatsapp_report.php?msg=success'</script>";
?>