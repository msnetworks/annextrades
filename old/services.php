...
if ($query != '') {
	if ($_SESSION['language'] == 'english') {
		...
		$select = "SELECT *,product.id as proid,product.country as countyid,registration.id as regid from product,registration,category where $query and product.status='2' and product.lang_status='3' and category.c_id=product.p_category and product.p_category ='394' and registration.companyname!='' and registration.id=product.userid order by udate desc ";
	}
} else {
	if ($_SESSION['language'] == 'english') {

		$select = "SELECT *,product.id as proid,product.country as countyid,registration.id as regid from product,registration,category where product.status='2' and product.lang_status='0' and category.c_id=product.p_category and product.p_category ='394' and registration.companyname!='' and registration.id=product.userid order by udate desc ";
	} else {
		$select = "SELECT *,product.id as proid,product.country as countyid,registration.id as regid from product,registration,category where product.status='2' and product.lang_status='3' and category.c_id=product.p_category and product.p_category ='394' and registration.companyname!='' and registration.id=product.userid order by udate desc ";
	}
}
...
?>
<div class="items-row-wrapper services-page">
	<div class="container">
		<!--div class="page-breadcrumb">
			<ul class="list-unstyled">
				<li><a href="#">Services</a><?php if (count($categoryCount) > 0) { ?> > <?php } ?> </li>
		...
		<div class="item-flex-row">
			...
			?>
					<div class="item-box">
						<figure><a href="servicecompanyinfo.php?id=<?php echo $product['id']; ?>&amp;cid=<?php echo $product['p_category']; ?>&amp;scid=<?php echo $product['p_subcategory']; ?>"><img src="<?= $image ?>" alt="<?php echo $product['p_name']; ?>"></a></figure>
						<div class="item-box-info">
							<h6><b><a href="servicecompanyinfo.php?id=<?php echo $product['id']; ?>&amp;cid=<?php echo $product['p_category']; ?>&amp;scid=<?php echo $product['p_subcategory']; ?>"><?php echo $product['p_name']; ?></a></b></h6>
							<div style="height: 70px; font-weight: 100;">
								<div class="text3">
									<font class="text-left">
										<?php 
										 echo  htmlspecialchars_decode(html_entity_decode(htmlentities($product['p_bdes'])));
										?>	
									</font>
								</div>
							</div>	
							 <div class="item-price-range" style="padding-top: 15px;">
								<strong>
									<?php echo (($product['p_price'] == 'USD' || $product['p_price'] == '') ? '$' : $product['p_price']) . "" . formatPrice($product['range1']) . " - " . formatPrice($product['range2']); ?>/-
								</strong>
							</div> 
							<div class="item-min-order"><small>
									<?php if (isset($product['p_min_quanity']) && !empty($product['p_min_quanity'])) : ?>
										<p class="min-order"><?php /* echo $product['p_min_quanity']; */ ?>Estimated Price</p>
									<?php endif; ?>
								</small></div>
							<div>
								<a class="item-contact-btn" href=<?php if ($sess_id != "") {
									if ($uid == $sess_id) {
								?> "#" onclick="return checking();" <?php
								} else {
									?> "proaction1.php?id=<?php echo $product['proid']; ?>" 
								<?php	}	} else { ?> "login.php" <?php
									}
								?>>Contact Now</a>
							</div>
						</div>
