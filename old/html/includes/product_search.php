...
<table width="525" border="0" cellspacing="0" cellpadding="0">
	...
	<td class="selectType">
      ...
      <td width="100" class="cate-listbg"><select name="country" id="country" class="cate-list">
		...
		{
		$select_coonnn="SELECT * FROM country_french";
		}
		else if($_SESSION['language']=='chinese')
		{
		$select_coonnn="SELECT * FROM country_chinese";
		}
		else 
		{
		$select_coonnn="SELECT * FROM country_spanish";
		}
		//$select_coonnn="SELECT * FROM country";
	   ...
	   <td width="100" class="cate-listbg"><select name="category" id="category" class="cate-list">
		...
		if($_SESSION['language']=='english')
			...
			{
			$select_catttee="SELECT * FROM category_french WHERE parent_id=''";
			}
			else if($_SESSION['language']=='chinese')
			{
			$select_catttee="SELECT * FROM category_chinese WHERE parent_id=''";
			}
			else 
			{
			$select_catttee="SELECT * FROM category_spanish WHERE parent_id=''";
			}
		//$select_catttee="SELECT * FROM category WHERE parent_id=''";
