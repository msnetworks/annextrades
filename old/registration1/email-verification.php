<?php 
    ...
    if ( $match ) {
        ...
        $email_update = mysqli_query($con, "UPDATE registration SET email_verify = 'Verified', userstatus = '0' WHERE vendor_id = '$vendor_id'");
        if ($email_update) { */
        
        echo "<script>alert('Your Email Has Been Verify Successfully.');</script>";
        header("location:./?vendor_id=$vendor_id&package=$package");
        /* if ($_GET['source']=='Annextrades') {
        }else{
            header("location: https://annexis.com/email-verify.php?vendor_id=$vendor_id&verify_code=$email_code&package=$package");
        } */
        ...
        exit();
            ...
            $query1 = mysqli_query($con, "SELECT * FROM registration WHERE vendor_id = '$vendor_id' and payment = 'No'");
            ...
            echo "<script>location.href='registration/?vendor_id=$vendor_id&package=$package'</script>";

            }

            else {
                echo "<script>alert('Your Registration is Already Completed.');</script>";
                echo "<script>location.href='https://annextrades.com'</script>";
            } 
